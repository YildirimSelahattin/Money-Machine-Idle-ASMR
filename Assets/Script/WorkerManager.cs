...
public class WorkerManager : MonoBehaviour
{
    ...
    public void MoveMachineAndComeBackByIndex()
    {
        ...
        transform.DOLocalMove(Spawner.Instance.firstRoadBreakdown[indexThatWorkerGoing % 2].position,
            firstPartLength / (GameDataManager.Instance.workerBaseSpeed )).SetEase(Ease.Linear).OnComplete(() =>
        {
            transform.DOLocalRotate(new Vector3(0, 0, 0), 0.1f);
            transform.DOLocalMove(
                machineObject.transform.parent.GetChild(GameManager.Instance.GRID_LAST_BREAKPOINT_INDEX).transform
                    .position,secondPartLenght / GameDataManager.Instance.workerBaseSpeed).SetEase(Ease.Linear).OnComplete(
                () =>
                {
                    if (indexThatWorkerGoing % 2 == 0)
                    {
    ...
    public void GoBackToPile(bool deployedSuccesfully)
    {
        ...
        if (deployedSuccesfully == false)
        {
            ...
            transform.DOLocalMove(Spawner.Instance.firstRoadBreakdown[indexThatWorkerGoing % 2].position,
                    Vector3.Distance(transform.position,
                        Spawner.Instance.firstRoadBreakdown[indexThatWorkerGoing % 2].position) / GameDataManager.Instance.workerBaseSpeed*1.7f)
                .SetEase(Ease.Linear)
                .OnComplete(() =>
                {
                    if (indexThatWorkerGoing % 2 == 0)
                    ...
                    transform.DOLocalMove(Spawner.Instance._spwanPos, Vector3.Distance(Spawner.Instance._spwanPos,transform.position)/ GameDataManager.Instance.workerBaseSpeed * 1.7f).SetEase(Ease.Linear).OnComplete(() =>
                    {
        ...
        else
        {
            ...
            transform.DOLocalMove(
                    ...
                    Vector3.Distance(
                        GameManager.Instance.gridParent.transform.GetChild(indexThatWorkerGoing)
                            .GetChild(GameManager.Instance.GRID_LAST_BREAKPOINT_INDEX).position, transform.position) / GameDataManager.Instance.workerBaseSpeed * 1.7f)
                .SetEase(Ease.Linear)
                .OnComplete(() =>
                {
                    ...
                    transform.DOLocalMove(Spawner.Instance.firstRoadBreakdown[indexThatWorkerGoing % 2].position,
                        ...
                        .OnComplete(() =>
                        {
                            ...
                            transform.DOLocalMove(Spawner.Instance._spwanPos, Vector3.Distance(Spawner.Instance._spwanPos,transform.position) / GameDataManager.Instance.workerBaseSpeed * 0.5f).SetEase(Ease.Linear)
                                .OnComplete(() =>
                                {
                                    if (waitingForGridDecision == false)
                                    {
